...
echo "=========================="
echo "Installing App cbftesting"

# Here is where we perform installation of scripts, services, etc.
echo " - Installing ROS packages for cbftesting..."
...
runuser -l $LIBPANDA_USER -c /etc/libpanda.d/apps/cbftesting/installRosPackages.sh
...
rosrun robot_upstart install cbf_testing/launch/${LAUNCH_FILE} --user root

echo "Enabling can_to_ros startup script"
sudo systemctl daemon-reload
sudo systemctl enable cbf
popd
echo "@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@@"
